...
{
	std::fstream fout("unknown.txt");
	if (fout.fail())
	{
		throw std::string("unable to save unknown input to list");
	}
...
{
	std::fstream fin("knowledgeBase.txt", std::ios::in);
	if (fin.fail())
	{
		throw std::string("Unable to load the database: knowledgeBase.txt");
	}
